...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
        <NavLink to='/' end className='flex flex-col items-center gap-1'>
          {({ isActive }) => (
            <>
              <p>HOME</p>
              <hr className={`w-2/4 h-[1.5px] bg-gray-700 ${isActive ? "block" : "hidden"}`} />
            </>
          )}
        </NavLink>
      ...
      <div className={`absolute top-0 right-0 h-screen z-50 bg-white transition-all duration-300 overflow-hidden ${visible ? 'w-full' : 'w-0'}`}>
